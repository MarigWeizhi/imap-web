							...
							try {
								if (!thisTemp.isUndefined(data['front']) || !thisTemp.isUndefined(data['pause']) || !thisTemp.isUndefined(data['end']) || !thisTemp.isUndefined(data['insert']) || !thisTemp.isUndefined(data['other'])) {
									...
									if (!thisTemp.isUndefined(val)) {
										neqingfeng['front'] = val;
									}
									val = thisTemp.arrayDel(data['pause']);
									if (!thisTemp.isUndefined(val)) {
										neqingfeng['pause'] = val;
									}
									val = thisTemp.arrayDel(data['insert']);
									if (!thisTemp.isUndefined(val)) {
										neqingfeng['insert'] = val;
										if (!thisTemp.isUndefined(data['inserttime'])) {
											neqingfeng['inserttime'] = thisTemp.arrayInt(data['inserttime']);
											isAdvShow = [];
											for (i = 0; i < neqingfeng['inserttime'].length; i++) {
												isAdvShow.push(false);
											}
											neqingfeng['insertPlay'] = isAdvShow;
										}
									}
									...
									if (!thisTemp.isUndefined(val)) {
										...
										neqingfeng['othertime'] = arrTemp;
										neqingfeng['otherPlay'] = isAdvShow;
									}
								}
							} catch(event) {
								thisTemp.log(event)
							}
							thisTemp.advertisements = neqingfeng;
							//对广告进行分析结束
						}
						thisTemp.getVideo();
					}
